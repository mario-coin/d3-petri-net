...
(function() {
  // dados iniciais
  var $tbody_arc, $tbody_places, $tbody_transition, add_row, build, build_delete_row, json;
  ...
  json.nodes.map(function(j, i) {
    // console.log(i, j)
    if (j.group === 0) {
      return $tbody_places.append(['<tr> <td> <input class="dyn-input" type="text" id="id" name="id" value="' + j.id + '" /> </td> <td> <input class="dyn-input" type="text" id="name" name="name" value="' + j.name + '" /> </td> <td> <input class="dyn-input" type="text" id="count" name="count" value="' + j.count + '" /> </td> <td> <button class="button tiny secondary delete-row">x</button> </td> </tr>']);
    } else if (j.group === 1) {
      return $tbody_transition.append(['<tr> <td> <input class="dyn-input" type="text" id="id" name="id" value="' + j.id + '" /> </td> <td> <input class="dyn-input" type="text" id="name" name="name" value="' + j.name + '" /> </td> <td> <button class="button tiny secondary delete-row">x</button> </td> </tr>']);
    }
  });

  // escrever na tabela arcos
  json.edges.map(function(j, i) {
    // console.log(i, j)
  ...
  build = function() {
    var $d3, $jq, AND, active, all, box_color, circs, click_color, color, dead_color, force, height, holds, incoming, links, live_color, node, outgoing, radius, rects, svg, tasks, texts, width;
    $d3 = d3;
    $jq = $; // jquery
    width = $jq("#rhs").width();
    ...
    $($tbody_arc.children('tr')).map(function(j, i) {
      return json.edges.push({
        source: +$(i).find('#source').val(),
        target: +$(i).find('#target').val()
      });
    });
    // rodar rede petri
